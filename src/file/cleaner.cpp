...
bool File::selectSegments(int segmentsToFree){
        ...
        for (int i = 1; i < log.super_block.segmentCount; i++) {
            double utilization = ((log.super_block.blocksPerSegment - log.SummaryBlockSize()) - log.GetFreeBlockCount(i))/ (double)(log.super_block.blocksPerSegment - log.SummaryBlockSize());
            struct tm y2k = {0};
            double seconds;
            ...
            ratios[i-1].age = difftime(now, newest);

            ratios[i-1].score = (((1 - utilization) * ratios[i-1].age) / (1 + utilization));
        }
        ...
        for (int i = 0; i<segmentsToFree; i++){
            Color::Modifier yel(Color::FG_YELLOW);
            Color::Modifier def(Color::FG_DEFAULT);
            std::cout << yel << "[CLEANER] SEGMENTS CHOSEN TO CLEAN:" << def <<std::endl;
            std::cout << yel << "[CLEANER] segment number: " << ratios[i].segmentNumber  << def <<std::endl;
            std::cout << yel << "[CLEANER] utilization: " << ratios[i].utilization  << def <<std::endl;
            std::cout << yel << "[CLEANER] last modified time: " << ratios[i].newest  << def <<std::endl;
            std::cout << yel << "[CLEANER] age: " << ratios[i].age  << def <<std::endl;
